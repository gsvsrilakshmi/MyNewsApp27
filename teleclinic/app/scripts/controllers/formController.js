...
angular.module('telehealthApp')
  .controller('formController', function (restcalls,$scope, $filter, $rootScope, $state) {
    ...
    $scope.getSlotsByLocation = function(){
        ...
        angular.copy( $scope.secondWeekData, firstWeekDataOriginal);
         angular.copy( $scope.thirdWeekData ,secondWeekDataOriginal);
          ...
          // $rootScope.$on('$locationChangeStart', function(next,event,current) {
          //       if(next==current && next=='/reasons' || next==current && next=='/schedule' || next==current && next=='/confirmDetails') {
